...
class Attention(nn.Module):
    ...
    def forward(self, x, h, w,return_attention=False ,Attention_mask=None):
        if (
        ...
        attn_score = torch.einsum('bhlk,bhtk->bhlt', [q, k]) 
...
class Block(nn.Module):
    ...
    def forward(self, x, h, w,return_attention=False,Attention_mask=None):
        ...
        x = self.norm1(x)
        attn = self.attn(x, h, w,return_attention=return_attention,Attention_mask=Attention_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x,return_attention=False,Attention_mask=None):
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x, H, W,return_attention=return_attention,Attention_mask=Attention_mask)
            if self.cls_token is not None:
...
class ConvolutionalVisionTransformer(nn.Module):
    ...
    def forward_features(self, x,return_attention=False):
        ...
        for i in range(self.num_stages):
            if return_attention:
                x, cls_tokens, patches, class_tokens = getattr(self, f'stage{i}')(x,return_attention=return_attention,Attention_mask=self.Attention_mask)
            else:
        ...
        #         Attention_mask.append(attentions[i].permute(0,2,3,1))
        #     return Attentions
        if return_attention:
    ...
    def forward(self, x,return_feat=False,return_attention=False):
        ...
        outputs = self.forward_features(x,return_attention=return_attention)
        if return_attention:
            return outputs
