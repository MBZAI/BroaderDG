...
# biobert_mlp_model = biobert_mlp_model.to("cuda")
...
class Biomedical_Clip_train_label_smoothing(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # smoothed_y = misc.label_smoothing(all_y)
...
#         all_x = torch.cat([x for x, y in minibatches])
#         all_y = torch.cat([y for x, y in minibatches])
#         loss = F.cross_entropy(self.predict(all_x), all_y, weight = self.weights_for_balance.to('cuda'))
...
#         self.Class_names=misc.Class_names
#         with torch.no_grad():
#             sentences = [tokenizer(f"a photo of a {c} case",  max_length=16, padding='max_length', truncation=True, return_tensors="pt")['input_ids'] for c in self.Class_names]
#             # print(sentences)
#             text_inputs  = torch.cat(sentences)
#             text_inputs = text_inputs.to('cuda')
#             outputs = biobert_mlp_model(text_inputs)
